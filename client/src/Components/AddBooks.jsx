...
export default function AddBooks() {
  ...
  async function addBook(e) {
    ...
    try {
      const response = await LibrarianApi.post("/add-book", formDataObject, {
        headers: {
  ...
  async function updateBook(e) {
    ...
    try {
      /**UPDATE BOOK */
      const response = await LibrarianApi.put("/update-book", formDataObject, {
        headers: {
          Authorization: `Bearer ${user.token}`,
          "Content-Type": "application/json",
        },
  ...
  return (
    <div className={`body-area`}>
      ...
      >
        ...
        ></textarea>
        <Button type={`submit`} text={editMode ? "Update Book" : `Add Book`} />
      </form>
    </div>
  );
}
